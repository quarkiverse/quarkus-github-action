...
<6> Install a Java 21 runtime configured for GitHub. It is important as it comes with a specific `~/.m2/settings.xml` allowing authenticated access to GitHub-hosted Maven repositories.
<7> Install JBang, which is used to run the jar file.
<8> Execute the jar file. It is downloaded from the Maven repository of the action GitHub repository.
