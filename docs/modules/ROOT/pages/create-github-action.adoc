...
[WARNING]
====
The workflow above will malfunction when used in a private repository. The automatically generated `GITHUB_TOKEN` only grants access to packages, that are part of the https://docs.github.com/en/packages/learn-github-packages/about-permissions-for-github-packages#access-tokens[same repository] the workflow is defined in.

To use a _GitHub Action_ defined in a separate private repository, belonging to the same user or organization, please follow these steps:

- Create or use a https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens[Personal Access Token] with `read:packages` scope. Depending on the actions to be performed, additional permissions can be necessary.
...
- The name you give the secret (e.g. `MY_ACTION_TOKEN`) must in turn be referenced in the workflow configuration above: Replace  &#x2780; with `github-token: ${{ secrets.MY_ACTION_TOKEN }}`.

After applying these steps, the workflow should be executed without any problems.
